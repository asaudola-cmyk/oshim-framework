PHP_ARG_ENABLE([oshim],
  [whether to enable the Sovereign OSHIM SAPI],
  [AS_HELP_STRING([--enable-oshim],
    [Enable building the Sovereign OSHIM Native SAPI Engine])],
  [yes],
  [no])

AC_MSG_CHECKING(for Sovereign OSHIM SAPI build)
if test "$PHP_OSHIM" != "no"; then
  PHP_ADD_MAKEFILE_FRAGMENT($abs_srcdir/sapi/oshim/Makefile.frag)

  dnl Output binary target
  SAPI_OSHIM_PATH=sapi/oshim/oshim

  dnl Register SAPI in build system
